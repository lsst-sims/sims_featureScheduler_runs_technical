...
if __name__ == "__main__":
    ...
    # Generate pregap only list vias:
    # ls *.txt | xargs -I % sh -c 'grep pregap % > pg_%'
    files = ['pg_' + filename for filename in files]

    ddf_file = files[filenumber]
    ...
    fileroot = 'ddf_mix_fn%i_' % filenumber
    file_end = 'v1.7_'
    ...
    euclid_detailers = [detailers.Camera_rot_detailer(min_rot=-camera_ddf_rot_limit, max_rot=camera_ddf_rot_limit),
                        detailers.Euclid_dither_detailer()]
    classic_ddfs = generate_dd_surveys(nside=nside, nexp=nexp, detailers=details, euclid_detailers=euclid_detailers)

    surveys = [ddfs, classic_ddfs, blobs, greedy]
