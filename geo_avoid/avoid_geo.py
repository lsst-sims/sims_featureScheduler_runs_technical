...
from lsst.sims.featureScheduler.surveys import (Greedy_survey,
                                                Blob_survey)
from dd_surveys import generate_dd_surveys
...
class Geo_mask_basis_function(bf.Base_basis_function):
    """
    """
    def __init__(self, nside=32, shadow_minutes=45, step=5):

        super(Geo_mask_basis_function, self).__init__(nside=nside)

        self.result = np.zeros(hp.nside2npix(self.nside), dtype=float)
        self.shadow_minutes = shadow_minutes*np.pi/60./12.
        self.step = step*np.pi/60./12.

    def _calc_value(self, conditions, indx=None):

        alt = conditions.alt
        az = conditions.az + 0
        az[np.where(az > np.pi)] = az[np.where(az > np.pi)] - 2*np.pi
        result = self.result.copy()
        if self.shadow_minutes == 0:
            steps = [0]
        else:
            steps = np.arange(0, self.shadow_minutes+self.step, self.step)
        for step in steps:
            val = np.abs(60. - 30.*(np.abs(np.degrees(az+step))/60.)**2-np.degrees(alt))
            in_geo = np.where(val < 10.)[0]
            result[in_geo] = np.nan
...
def gen_greedy_surveys(nside=32, nexp=1, exptime=30., filters=['r', 'i', 'z', 'y'],
    ...
    for filtername in filters:
        ...
        bfs.append((Geo_mask_basis_function(nside=nside, shadow_minutes=0., step=0.), 0))
...
def generate_blobs(nside, nexp=1, exptime=30., filter1s=['u', 'u', 'g', 'r', 'i', 'z', 'y'],
    ...
    for filtername, filtername2 in zip(filter1s, filter2s):
        ...
        bfs.append((bf.Planet_mask_basis_function(nside=nside), 0.))
        bfs.append((Geo_mask_basis_function(nside=nside, shadow_minutes=45.), 0))
